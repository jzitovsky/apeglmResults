---
title: "Simulation with covariates"
...
quantile(shrink, prob=c(.5,.75,.9,.95,.975,.99,.995))   #quantiles of shrinkage
quantile(shrinkash, prob=c(.5,.75,.9,.95,.975,.99,.995))#ash has universally greater shrinkage

hist(shrink[shrink<2], ylim=c(0,500), main=NULL); title(main="histogram of shrinkage for apeglm (y-axis restricted to [0,500]")   
plot(shrink ~ truth); title(main="shrink vs. true effect plots for apeglm")
#hist(shrinkash[shrinkash<2], ylim=c(0,500))  
#plot(shrinkash ~ truth)                                #very large shrink cases                  
plot(shrink[shrink<2] ~ truth[shrink<2]); title(main = "shrink vs. true plots, y-axis restricted from 0 to 2")
#plot(shrinkash[shrinkash<2] ~ truth[shrinkash<2])      
#we can see that both ash and apeglm shrink estimates where true effects are close to zero
```

# improvement statistics and plots
...
```{r, echo=F}
#improvevement statistics
cat("mean apeglm improvement for shrunk genes\n")
mean(improve[shrunk])                                   
cat("mean ash improvement for shrunk genes\n")
mean(improveash[shrunkash])      
cat("quantiles of improvement with shrunk genes for apeglm (top) and ash (bottom)\n")
quantile(improve[shrunk], prob=c(0.005,0.01,0.025,0.05,0.1,0.25,.5,.75,.9,.95,.975,.99,.995)) #quantiles of improvement
quantile(improveash[shrunkash], prob=c(0.005,0.01,0.025,0.05,0.1,0.25,.5,.75,.9,.95,.975,.99,.995)) #improvement more extreme in both directions (as greater shrinkage) with equal median, but MEAN is higher
cat("Mean absolute error\n")
getMAE(goal = truth, mle = mle, map = map, ash = ashe, int = TRUE)
cat("MAE and MAD for genes shrunk by apeglm (recall we define shrunk as >0.1 shrinkage)\n")
getMAE(goal = truth, mle = mle, map = map, ash = ashe, int = shrunk)
getMAD(goal = truth, mle = mle, map = map, ash = ashe, int = shrunk)
cat("MAE and MAD for genes shrunk by ash\n")
getMAE(goal = truth, mle = mle, map = map, ash = ashe, int = shrunkash)
getMAD(goal = truth, mle = mle, map = map, ash = ashe, int = shrunkash) #on average, ash has (slightly) better MAE and MAD

improve2 = ifelse(improve< -1, -1.1, improve)
improve3 = ifelse(improve2>1, 1.1, improve2)
improveash2 = ifelse(improveash< -1, -1.1, improveash)
improveash3 = ifelse(improveash2>1, 1.1, improveash2)
hist(improve3[shrunk], breaks=10, main=NULL); title(main="histogram of apeglm improvement (movement from MLE to truth) for shrunk genes", xlab = "improvement (very large/small improvement changed to values of 1.1 and -1.1")                       
hist(improveash3[shrunkash], breaks=10, main=NULL); title(main="histogram of improvement (movement from MLE to truth) for ash shrunk genes", xlab = "improvement (very large/small improvement changed to values of 1.1 and -1.1")  
```
...
```{r, echo=F}
#other statistics - interval accuracy and width
...
cat("proportion of times true effects were in the 95% confidence intervals for MLE, MAP and ash (in that order) \n")
sum(truemleCI)/length(truemleCI)
sum(truemapCI)/length(truemapCI)
sum(trueashCI)/length(trueashCI)
cat("summary statistics for standard errors of MLE, MAP and ash estimates (in that order) \n")
...
getFilterRule(sub, beta, 4, max=1000, all=TRUE, load=FALSE, desc="filtering rule: genes with less than this many total counts removed")

filterRule1 = filterIt(pSub, 4, 40)
filterRule2 = filterIt(pSub, 8, 20)
filterRule3 = pSub$counts$total>610
cat("number of genes remaining when filtering no genes, filtering by half samples, filtering by all samples and filtering by total counts, in that order \n")
...
cat("Mean absolute error\n")
getMAE(goal = truth, mle = mle, map = map, ash = ashe, int=TRUE)
cat("MAE for apeglm-shrunk genes (shrinkage>0.1) \n")
getMAE(goal = truth, mle = mle, map = map, ash = ashe, int=shrunk)
cat("MAE for ash-shrunk genes (shrinkage>0.1) \n")
getMAE(goal = truth, mle = mle, map = map, ash = ashe, int=shrunkash)
cat("MAE for MLE with filtering \n")
...
```{r, echo=F}
cat("Quantiles of apeglm shrinkage\n")
quantile(shrink, prob=c(.5,.75,.9,.95,.975,.99,.995)) 
cat("Quantiles of ash shrinkage\n")
quantile(shrinkash, prob=c(.5,.75,.9,.95,.975,.99,.995))  
cat("MAE\n")
getMAE(goal = truth, mle = mle, map = map, ash = ashe, int = TRUE)
...
getFilterRule(sub, beta, 5, load=FALSE, desc="filtering rule: genes with less than this many counts for half the samples removed")
getFilterRule(sub, beta, 10, load=FALSE, max=100, desc="filtering rule: genes with less than this many counts for all the samples removed")
getFilterRule(sub, beta, 5, max=1000, all=TRUE, load=FALSE, desc="filtering rule: genes with less than this many counts combined removed")
...
#Filtering effects
getFilterRule(sub, beta, 3, load=FALSE, desc="filtering rule: genes with less than this many counts for half the samples removed")
getFilterRule(sub, beta, 6, load=FALSE, max=100, desc="filtering rule: genes with less than this many counts for all the samples removed")
getFilterRule(sub, beta, 3, max=1000, all=TRUE, load=FALSE, desc="filtering rule: genes with less than this many counts combined removed")
